...
- `ENABLE_WHATSAPP_SYNC`: Enable WhatsApp linked-device sync (default: false). Requires `ENABLE_EXTERNAL_SYNC=true` — startup fails when this is enabled without it. The dependency is one-way: external sync alone is a normal configuration. Tuning knobs: `WHATSAPP_BURST_WINDOW_HOURS` (2), `WHATSAPP_REPLY_BRIDGE_HOURS` (48), `WHATSAPP_DISCOVERY_MIN_MESSAGES` (3), `WHATSAPP_GROUP_MAX_MEMBERS` (10)
