-- Keep contact_method.value_normalized in sync on insert/update.
...
CREATE OR REPLACE FUNCTION set_contact_method_value_normalized()
RETURNS TRIGGER AS $$
BEGIN
    NEW.value_normalized := normalize_contact_method_value(NEW.type, NEW.value);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
WHERE value_normalized IS NULL OR value_normalized = '';
