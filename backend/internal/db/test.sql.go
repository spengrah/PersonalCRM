...
const SyntheticDeleteIdentitiesForOwnedCandidates = `-- name: SyntheticDeleteIdentitiesForOwnedCandidates :execrows
...
const SyntheticDeleteTitleCandidatesForHostSessions = `-- name: SyntheticDeleteTitleCandidatesForHostSessions :execrows
DELETE FROM external_contact
WHERE source = 'anarlog_title'
  AND metadata->>'session_uuid' IN (
    SELECT mn.anarlog_session_id::text FROM meeting_note mn
    WHERE mn.mac_host_id = $1
  )
`
...
const SyntheticSelectLinkedContactIdsForHostSessionTitleCandidates = `-- name: SyntheticSelectLinkedContactIdsForHostSessionTitleCandidates :many
SELECT DISTINCT ec.crm_contact_id FROM external_contact ec
...
type SyntheticSelectLinkedContactIdsForHostSessionTitleCandidatesParams struct {
	Hostname  string `json:"hostname"`
	Namespace string `json:"namespace"`
}
...
const TestGetViewDefAndOptions = `-- name: TestGetViewDefAndOptions :one
...
type TestGetViewDefAndOptionsRow struct {
...
func (q *Queries) TestGetViewDefAndOptions(ctx context.Context, viewName string) (*TestGetViewDefAndOptionsRow, error) {
	row := q.db.QueryRow(ctx, TestGetViewDefAndOptions, viewName)
	var i TestGetViewDefAndOptionsRow
	err := row.Scan(&i.ViewDefinition, &i.Reloptions)
	return &i, err
}
