...
// SyncLogTrimArgs are the args for the periodic SyncLogTrimWorker. Stateless
// like the janitor; the retention window is worker configuration, not a job
// argument, so the args type is intentionally empty.
type SyncLogTrimArgs struct{}

// Kind implements river.JobArgs.
func (SyncLogTrimArgs) Kind() string { return "sync_log_trim" }
