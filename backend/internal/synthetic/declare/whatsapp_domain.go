...
func init() {
	...
	RegisterNone("WHA-077", "both citing tests fulfil /whatsapp/auth/status with terminal payloads — a logged-out disconnect, a temporary ban with its lift time, and a startup error. Those states are reached only by WhatsApp itself ending the session or the stack failing to come up")
}
