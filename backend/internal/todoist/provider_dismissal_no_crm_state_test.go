...
func TestCadenceTaskDeadline(t *testing.T) {
	...
	negativeOffset := time.FixedZone("negative-offset", -7*60*60)
	todayLocal := time.Date(2026, time.September, 15, 0, 0, 0, 0, negativeOffset)
	contactByDate := time.Date(2026, time.September, 15, 0, 0, 0, 0, time.UTC)
	assert.Equal(t, "2026-09-15", cadenceTaskDeadline(contactByDate, todayLocal))
}
...
func TestHandleSkipTrigger_ReplacementSurvivesNextReconcile(t *testing.T) {
	...
	contact, _ := createDismissalContact(t, env, "SkipReplacementReconcile")
	...
	require.NoError(t, err)
	assert.Equal(t, skippedTo, updated.Metadata["synced_contact_by"])
}
