...
qa-obs-smoke: ## Live-verify the usage generation observation end-to-end against a WRITABLE Langfuse. Ships ONE synthetic span (behavior SMOKE-OBS, per-invocation ids stable within the run, fixed token counts) twice, reads the trace back, and asserts observations non-empty, usageDetails echoed exactly, totalCost == the hand-computed figure, startTime from the span (not export time), and a stable id across the re-export. Requires LANGFUSE_HOST/PUBLIC_KEY/SECRET_KEY with WRITE access. Non-zero exit on any failed assertion.
