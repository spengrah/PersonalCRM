...
> ⚠️ **The old native deploy path is retired.** Post-cutover deploys go through the promotion flow: merge to `develop`, then `make promote` fast-forwards `main`, which triggers the prod deploy on the self-hosted runner (`.github/workflows/deploy-prod.yml` → `deploy-artifact.sh`, which swaps the Podman image and runs migrations). The legacy rsync-based `scripts/deploy.sh` / `make deploy` no longer exist — they reinstalled + restarted the `personalcrm-*` **system** units, which bound the same loopback ports (8080/3001) as the Podman containers and would have caused split-brain / port collision against this Quadlet stack.
