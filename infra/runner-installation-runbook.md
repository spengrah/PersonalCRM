...
Throughout: `$PI_HOST` is the Pi's SSH host alias; `crm` is the existing service user (uid 995, system account, home `/var/lib/personalcrm`, linger on) that owns the rootless Podman runtime. The deploy automation introduces a SECOND, separate identity — `gha-runner` — which runs the GitHub Actions agent and whose only privilege is to invoke one immutable, reviewed script as root. The two identities are deliberately distinct: the runner (untrusted CI surface) is never `crm` and never root; it can only call `deploy-artifact.sh`, which internally hops to `crm` for every rootless op.
...
- **Updating a deployed script is the SAME `scp` + `install` sequence** (it is idempotent — `install` overwrites in place, preserving `root:root` `0755`). Merging a PR that edits `deploy-artifact.sh` / `backup-db.sh` / `restore-db.sh` does NOT update the Pi: the installed copy at `/usr/local/sbin/` is a point-in-time snapshot, so re-run the copy-from-checkout block above (for the changed script) to apply the new version. Until then the old installed copy keeps running. For example, the health-gate commit verification added for #451 (the gate cross-checks `/health`'s `git_commit` against the deployed SHA) is inert until `deploy-artifact.sh` is re-installed; the old gate keeps working and simply does not verify commits.
