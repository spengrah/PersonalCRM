...
export function TodoistAccountsSection() {
  ...
  const { data: settings, refetch: refetchSettings } = useTodoistSettings(hasAccount)
  ...
  const handleTriggerSync = async (accountId: string) => {
    setSyncingSource(`todoist-${accountId}`)
    try {
      await triggerSyncMutation.mutateAsync({ source: 'todoist', accountId })
      setNotification({ type: 'success', message: 'Todoist sync started!' })
      refetchSettings()
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (auth && provider === 'todoist') {
      if (auth === 'success') {
        setNotification({ type: 'success', message: 'Todoist account connected successfully!' })
        refetch()
      } else if (auth === 'error') {
        setNotification({
          type: 'error',
  ...
  return (
    <section className="bg-white rounded-lg shadow-sm border p-6">
      ...
      <p className="text-gray-600 mb-6">
        Connect your Todoist account to sync contact cadence tasks.
      </p>
      ...
      {showEmptyState && (
        <div className="py-12 text-center border-2 border-dashed border-gray-200 rounded-lg bg-gray-50">
          ...
          <p className="text-gray-500 mb-6 max-w-sm mx-auto">
            Connect your Todoist account to start syncing cadence tasks.
          </p>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {account.scopes?.includes('data:read_write') && (
                      <PermissionBadge label="Read & Write" />
                    )}
                    <SyncBadge
                      label="Tasks"
                      syncState={getSyncStateForAccount(syncStates, 'todoist', account.account_id)}
                      onSync={() => handleTriggerSync(account.account_id)}
                      loading={syncingSource === `todoist-${account.account_id}`}
                    />
                  </div>
                </div>
              </div>

              {/* Sync Configuration card (separate from account card per design) */}
              <div className="mt-4 p-4 rounded-lg border bg-white border-gray-200">
                <h3 className="text-sm font-semibold text-gray-900 mb-1">Sync Configuration</h3>
                <p className="text-xs text-gray-500 mb-4">
                  Select the Todoist project and label to use for cadence tasks.
                </p>
