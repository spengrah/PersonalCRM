...
- **A declaration spends from the composed world's BUDGETS.** `declare.World()` runs every registered declaration and edge into ONE namespace, so registering fixtures grows the `standard` world for everybody. The bounds are two-sided and asserted in the `budgets` subtest of `TestSyntheticDeclareStandardWorld`: contacts in `(50, 500]` (the tours resolve fixtures inside a `limit=500` window), the overdue population in `(50, synthetic.TourOverdueCaptureCap]` (a larger one reaches the judge as an unnamed subset of the captures), and drawn synthetic phones under 100 (the per-namespace block PANICS on exhaustion). That subtest is `RequireLongTests`-gated and both CI and pre-push run `test-integration-fast`, so a blown bound surfaces on the NIGHTLY cron rather than on the PR that caused it — a PR that registers overdue-bearing declarations should run it once (`make test-integration INTEGRATION_PKGS=./tests/... INTEGRATION_RUN=TestSyntheticDeclareStandardWorld`, with `LONG_TESTS=1`) and report the new count. Raising a cap is a deliberate two-sided change (the constant AND the slicing it feeds), never a relaxed assertion.
- **`ExplicitName` pins a literal, and its uniqueness is a WHOLE-REGISTRY property.** It is exempt from the display-name dedupe on purpose, so registration rejects a repeated literal inside one entity list and a unit test rejects one across lists — two declarations sharing a name builder put identically-named contacts into the composed world, which is the ambiguity the exemption cannot resolve for itself. It is also mutually exclusive with `SameNameAs` and `NameEdge`, both of which would render something other than the pinned literal.
