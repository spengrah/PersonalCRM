...
The synthetic-seed toolkit (`backend/internal/synthetic/`) produces **production-shaped, PII-free, deterministic** data by replaying fake source input through the *real* ingestion pipeline. It does not just write terminal domain rows: it injects synthetic sync-source inputs (fake Gmail, GChat, GCal, Telegram, iMessage, Mac-Contacts, Todoist payloads) and drives them through provider normalization → matching → dedup → event bus → River consumers → downstream graph, so the full sync flow is exercised without live credentials. One library, reused four ways: unit tests use the factories directly; integration tests use the replay harness; staging and `make dev-seed` use the profile entrypoints; the QA harness tours the declared `standard` world.

## Why the seed drives services and provider replay, not the HTTP API

Seed the cause, not the endpoint — but one layer below the handlers. Most prod data has no API surface: interactions, external contacts, sync state and enrichment assertions originate from sync pipelines, so for them the app's real logic IS the replay path, and inventing endpoints would fake the sync result rather than reproduce its cause. A production-shaped world also needs what the API correctly forbids — contacts created months ago, interactions backdated across cadence windows, deterministic PRNG, per-test namespacing — so the seed injects timestamps at the service/repo layer while still driving the real causal chain from there down. The skipped layer is the thinnest one, DTO validation and auth, which E2E and the QA tours exercise against the seeded world anyway. The trade-off: a handler-only bug never corrupts the seeded world and is not caught here; service-layer bugs are.
...
## The data shapes — from one mechanism

A payload factory takes a `factory.MatchIntent`, and the dataset's states fall out of **whether a synthetic input's sender matches a seeded contact**, not separate code paths:
...
The harness's `t.Cleanup` deletes everything the run created.

## Declared seeding (`synthetic/declare`) — the E2E provisioning path

`backend/internal/synthetic/declare` is a layer ON TOP of the toolkit: a fixture is stated in DOMAIN terms and keyed by the SPEC BEHAVIOR ID it provisions, and the E2E suite asks for it by that id. It is the E2E provisioning path, and it replaces nothing in the toolkit — a declaration lowers onto the same factory + harness primitives.
...
// testApi.cleanup() removes the declared worlds, plus the contacts this test
// created through the PRODUCT (they carry the test's prefix in their name)
```

Load-bearing properties, none of them optional:

- **Every ui-surface spec behavior is resolved exactly once** — Declaration, `RegisterNone(reason)`, or a waiver in `declare/waivers.go`. A unit test in the existing `make test-unit` lane fails on anything unresolved, double-resolved, or naming a behavior that no longer exists. A `RegisterNone` reason must name the MECHANISM — which endpoint the citing assertions run against, or which other behavior's fixture they read — never a category like "route-mocked". Reserve a waiver for a behavior with no implementation AND no citing test.
...
Endpoints (test-only, gated on `CRM_ENV` ∈ {testing, test}): `POST /api/v1/test/seed/declared`, and `POST /api/v1/test/cleanup` in two shapes — `namespaces` removes the declared worlds, `prefix` hard-deletes the contacts a test created through the PRODUCT, which carry its prefix in their name and belong to no namespace. The declared handler drives `declare` directly under a documented layering exception: `service` cannot import `synthetic` without a cycle, and the toolkit writes through the real services one level down.

## Adding a factory for a new entity or source payload

Factories are pure functions of `(seed, namespace)` except for timestamps. Read `synthetic/factory/factory.go`: its package doc states the two determinism claims (wall-clock-independent identifiers vs anchor-relative timestamps) and the PII / isolation rules.
...
| WhatsApp (direct chats only) | `ReplayWhatsApp` | `whatsapp.Ingestor.IngestMessage` (the seam the live handler and the history drainer share) |
...
`ValidateNamespace` enforces the charset `^[a-z0-9-]+$` (rejecting the SQL `LIKE` metacharacters `_` and `%`), so the prefix-based cleanup deletes (`LIKE 'synth-<ns>-%'`) can never over-match another namespace. Each namespace gets **disjoint numeric bands** (telegram peer block, message-id block, phone area code) keyed by a hash of the namespace; `resolveNamespace` collision-checks them at setup and re-salts on collision. The guarantee is "probabilistically disjoint + detected at setup", not a mathematical one.

**Hard rule — no DB-wide enumerate-then-write.** No replay path may scan a table DB-wide and then mutate (e.g. "list all unmatched external_contacts, then reconcile them"). On the shared test DB that silently reaches across namespaces and corrupts a parallel run's data. Every settle read, cleanup delete, and reconcile must be scoped to *this* run's tracked ids or this namespace's prefix/band. The cleanup ledger (`created` in `replay.go`) exists precisely so deletes go by exact id, never by a DB-wide source/kind value.
...
Both budgets are **real wall-clock** (`context.WithTimeout`), not accelerated time — settle latency is infrastructure timing, and an accelerated budget would collapse under a high `TIME_ACCELERATION`. A gate timeout names the unmet gate and means a wiring regression, not normal latency.
...
The **failure-path contract** governs the unsettled case: teardown stops *this* harness's River client, bounded-waits Gate B, and gates the **entire** cleanup on Gate B == 0. If Gate B does not clear it **skips all deletes**, leaving the namespaced inert dataset for a DB reset to reclaim — because a retained unfinalized `river_job` picked up later by a shared default-queue client dereferences this replay's rows, and deleting them under a live job faults that worker. Non-test entrypoints call `Harness.Quiesce` on success (stop client + bounded-wait, no deletes) and the full teardown closure on error. Either way the River client is always stopped, never leaked.

## Requesting a scenario / profile

There are exactly **TWO** profiles (`synthetic/profiles.go` holds the dispatch core; `synthetic/standard.go` holds the declared world). `ProfileParams` refuses the retired `dev` and `prod-shaped` names with a loud "unknown profile" error, so an un-updated script fails rather than silently building a different world.

- `synthetic.ProfileStandard` (`"standard"`) — **the default** for `make dev-seed`, `make staging-reset`, the automated staging reseed, and the QA tours. The DECLARED world: every registered `declare` declaration (behavior-id sorted), then every adversarial edge (`declare.Edges()`, catalog registration order), then the pinned tour fixtures LAST. It carries **no `Counts`** — it is exactly what the two registries say it is, which is why its content is asserted as named states rather than as statistics. Adding a declaration or an edge joins it automatically, and spends from the world's budgets (see the declaration bullet above). `declare.World` does not trust its tail's reported manifest: it checks every reported contact id against `Harness.SeedContact`'s ledger, rejecting both a row reported after the pinned fixtures and a created-but-unreported one. On failure `Steps` keeps the completed prefix, `Current` names the failing step, and partial entities stay in `Order` so summaries report truthful partial counts.
...
Some pending states have **no toolkit producer yet**: `conflict_pending` meeting_note, title-candidate review rows, `comms_message` without `interaction_id`. Adding one means adding the factory/replay producer plus a declaration or adversarial edge asserting it through a read path.

## Asserting the seeded population's shape — read what the product reads

Assert what the seed produced through the same read the application performs, never through a recomputation of the concept from the underlying columns: the two differ whenever a derived column lags its inputs, and the recomputation passes while the deployed world disagrees. `standard`'s declarations and adversarial edges are asserted by the E2E specs that cite them, as named states — never as percentages or population bands.
...
| `scripts/check-tour-markers.sh` | Asserts every pinned tour-fixture marker resolves to EXACTLY ONE contact in the world currently served by `TOURS_API_URL`, using the same predicate `resolveFixture` uses and reading the marker tokens from the Go SSOT. Exits 1 naming every offending marker. Run it against a FRESH world BEFORE the tours; it is deliberately not wired into `run-tours.sh`, because the contacts tour CONSUMES `fxdeletevictim` and a second `TOURS_SKIP_RESET=1` pass would legitimately find zero. |
| `/test/seed/declared` + `/test/cleanup` routes | The E2E HTTP surface. Declared seeding and the `namespaces` cleanup shape drive `declare` from the handler; the `prefix` shape goes through `service.TestSeedService`, which does NOT import the synthetic package — that would close a `service → synthetic → replay → service` cycle. No handler calls sqlc queries. |

## Adding a source or a declarable entity class — the five audits

Fill each table once for the thing you are adding and record it in the PR body. One-time audits, not gates — commit no machinery.

| Audit | Enumerate | Defect it catches |
|---|---|---|
| **Source fidelity** | admissible shapes vs what the production writer can emit — its INPUT grammar too, not only its output | a shape the writer cannot produce is a fixture asserting a state the product cannot reach |
| **Metadata shapes** | every JSONB/metadata field, per source, against the builder's emit site | an omitted field makes a UI branch unreachable; a wrong layout makes a matcher silently miss |
| **Cleanup discoverability** | every row class AND side effect (identity rows, ownership records, post-import hooks, product-derived rows) × its recovery route | rows with no route survive teardown; enumerate the hooks, never sample |
| **Route composition** | each recovery route × each link-carrying delete | a harvest walking a link reaches a neighbouring world's row; exclude in the predicate, never in a comment |
| **Identifier allocation** | every identifier surface × its regime | band-allocated values need an occupancy check where namespace-embedded ones do not; disqualify opaque keys explicitly |

Two structural rules:

- **Replayability runs through production writers AND production inputs.** A fixture that writes rows directly, or feeds a writer an input its production caller could never produce, is not a replay.
- **One shared predicate, zero copies.** Occupancy, discoverability and same-world exclusion get one implementation with N callers. A second copy is the defect, not the safeguard.

Ask before claiming a fixture or a sweep is correct:

- Which assertion turns red if this fixture produces nothing? If none — an absence, emptiness, negative or one-sided-count assertion — falsify it ephemerally and discard the mutant.
- Can the production writer emit this shape? Can its production caller supply this input?
- If this row class leaked, which read would show it? Read residue from the database, never from the cleanup's own counts.
- Does the recovery token survive everything the product can do to the row? (`full_name` is user-editable, which is why ownership is recorded by id.)
- Does this delete walk a link out of the namespace?

## Slow-test routing

Replay integration tests are River-draining and gated out of the fast suite. Call `testsupport.RequireLongTests(t)` at the top of any replay test — it skips unless `LONG_TESTS` is set and short mode is off. The slow workflow (`make test-integration-slow`, run by `backend-slow-tests.yml`) sets `LONG_TESTS=1` and selects by `BACKEND_SLOW_TESTS_REGEX`, which includes `TestSynthetic` — so give synthetic replay tests that prefix. Both are load-bearing and do different jobs: the prefix is all the slow lane SELECTS on, the `RequireLongTests` call is all that makes the fast lane SKIP. A test that would only ever run on the nightly cron gates nothing: delete it rather than gate it. Factory-only unit tests need neither.
