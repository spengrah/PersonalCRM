...
- **No seeded display name is another one's SUBSTRING, and the drawn half of that rule is the dedupe.** Names are drawn WITH REPLACEMENT from a 16×10 pool, so a three-contact fixture repeated one ~1.8% of runs (measured) — enough to break any selector that resolves a contact BY NAME, which is how a manifest-driven spec addresses its fixtures (Playwright's strict mode fails outright on two matching headings). A repeat now carries its contact sequence number spliced BETWEEN the given name and the surname (`Glyph 3 Fixtura`), never appended: a trailing number would leave the earlier name a PREFIX of the later one, and `getByText` matches on substring, so the shorter name would resolve both rows while reading as a legitimate hit — and it would displace a declared `NameMarker` from the end of the name. Nothing else can nest one drawn name inside another (both carry the namespace prefix, and no pool given name or surname is a prefix of another), so the invariant is asserted directly, over a pool-exhausting run, in `TestContact_DisplayNamesStayUniquePastThePool`. It is disambiguated rather than redrawn on purpose: a redraw would consume extra rng values and shift every later draw, so a namespace that does NOT collide stays byte-identical. The same no-substring rule governs the other two name sources: pinned literals across the whole `declare` registry, and the pinned tour markers.
- **A declaration spends from the composed world's BUDGETS.** `declare.World()` runs every registered declaration and edge into ONE namespace, so registering fixtures grows the `standard` world for everybody. The bounds are two-sided and asserted in the `budgets` subtest of `TestSyntheticDeclareStandardWorld`: contacts in `(50, 500]` (the tours resolve fixtures inside a `limit=500` window), the overdue population in `(50, synthetic.TourOverdueCaptureCap]` (a larger one reaches the judge as an unnamed subset of the captures), and drawn synthetic phones under 100 (the per-namespace block PANICS on exhaustion). Measured occupancy at the contacts-domain migration: **contacts 188/500, overdue 66/96, phones 2/100** — the subtest logs that line, so a PR that grows the world can read its own numbers off the run. That subtest is `RequireLongTests`-gated and both CI and pre-push run `test-integration-fast`, so a blown bound surfaces on the NIGHTLY cron rather than on the PR that caused it — a PR that registers overdue-bearing declarations should run it once (`make test-integration INTEGRATION_PKGS=./tests/... INTEGRATION_RUN=TestSyntheticDeclareStandardWorld`, which already exports `LONG_TESTS=1`) and report the new count. Raising a cap is a deliberate two-sided change (the constant AND the slicing it feeds), never a relaxed assertion.
- **`ExplicitName` pins a literal, and its uniqueness is a WHOLE-REGISTRY property.** It is exempt from the display-name dedupe on purpose, so registration rejects a repeated literal inside one entity list and a unit test rejects one across lists — two declarations sharing a name builder put identically-named contacts into the composed world, which is the ambiguity the exemption cannot resolve for itself. That unit test rejects a SUBSTRING pair too (`Button Nav 1` beside `Button Nav 10`), for the same reason the drawn names avoid one. A cross-list check inside `Register` IS possible — an accumulate-and-compare sees every pair when its later member registers — and is deliberately not done: `Register` panics and `declare` is linked into `crm-api`, so a collision would abort the API at startup instead of failing a named test. It is also mutually exclusive with `SameNameAs` and `NameEdge`, both of which would render something other than the pinned literal. The read-path oracles for `ExplicitName` and `NameMarker` live in `TestSyntheticDeclareSatisfiability`, `RequireLongTests`-gated like the budgets subtest above — so an unwired lowering surfaces on the NIGHTLY cron, while the registry-level guards are unit tests in the fast lane.
...
- `synthetic.ProfileStandard` (`"standard"`) — **the default** for `make dev-seed`, `make staging-reset`, the automated staging reseed, and the QA tours. The DECLARED world: every registered `declare` declaration (behavior-id sorted), then every adversarial edge (`declare.Edges()`, catalog registration order), then the pinned tour fixtures LAST. It carries **no `Counts`** — it is exactly what the two registries say it is, which is why its content is asserted as named states rather than as statistics. Adding a declaration or an edge joins it automatically; the world's own integration test asserts the tour budgets two-sided (overdue population above fifty and at or under `synthetic.TourOverdueCaptureCap`, contacts inside the tours' `limit=500` selector window), so growth that would break a tour fails a named Go test rather than an opaque staging run.
- `synthetic.ProfileMinimalScoped` (`"minimal-scoped"`) — the smallest viable world (a few contacts, one settled interaction per source). The CI/E2E namespacing baseline; the golden test pins its shape, so it MUST stay byte-stable. It is an **explicit operator override**, never a default: a world this small has no content for a UI surface or a QA tour to exercise.
...
That whole class of statistical gate is now gone with the catalog profiles, and the rule it taught is what replaced it: `standard`'s declarations and adversarial edges are asserted through the API read path (`TestSyntheticDeclareStandardWorld`, `TestSyntheticDeclareEdges`), as named states and exact id sets rather than as percentages.
