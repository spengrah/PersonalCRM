...
| `make staging-reset` | Drives the rootless `staging` tenant (ssh `STAGING_HOST`, default `stovepipes`; `--local` on-box): stops `personalcrm-backend`, runs `crm-admin --reset-and-seed --profile prod-shaped --yes` from an ephemeral container off the deployed backend image (the unit's pinned `Image=`, never `:latest`/`podman exec`), restarts. Reads `CRM_ENV` from the deployed staging `.env`; fail-closed refuse if it is a production alias OR empty/unset. |
