...
The `TestAPI` constructor mints a per-worker prefix (`w{workerIndex}-{timestamp}`) once, exposed as `testApi.prefix`. It exists for rows the PRODUCT creates during a test: when a spec creates its own rows through the product's own API (a contact POST, a note PUT), build their identifying strings from `testApi.prefix` so `cleanup()` can find them, and build assertion strings from it rather than hardcoding a name. `seedBehavior` isolates differently and more strongly, and it is the only provisioning path: each call gets its OWN namespace (`${prefix}-c{n}`) and the generator owns every name, so assertions read them out of the returned manifest rather than composing them — see `declaredWorldNamePrefix` / `declaredWorldSearch` for the two scoping terms the UI surfaces need. `test.afterEach` calls `testApi.cleanup()`, which deletes only the contacts matching this test's name prefix plus every declared namespace it seeded — never a wildcard.
