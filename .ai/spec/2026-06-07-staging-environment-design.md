...
- [ ] **Runner install on the VPS.** Reproducible runbook mirroring `infra/runner-installation-runbook.md`: the `gha-runner` system user (≠ tenant, ≠ root), agent labeled `staging` (`runs-on: [self-hosted, staging]`), the `deploy-staging.sh` wrapper + co-installed (possibly parameterized) `backup-db.sh`/`restore-db.sh` (root-owned, runner-immutable), the single sudoers entry, the ntfy env file (degrade-open is acceptable on staging, unlike prod's mandatory gate).
