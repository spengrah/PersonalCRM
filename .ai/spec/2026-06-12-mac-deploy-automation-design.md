...
3. **CI gate** — query `ci.yml`'s conclusion for the target SHA via `gh api`. Auth differs by trigger: the **runner path** (`deploy-mac.yml`) uses the workflow `GITHUB_TOKEN` (`actions: read` + `GH_TOKEN`, mirroring `deploy-prod.yml`) because `gh` inside an Actions job does not use the user's keyring login; the **timer path** uses the user's existing `gh` keyring auth. Outcomes: `success` → proceed; `failure`/`cancelled` → fail-closed with a failure ntfy; **in-progress / not-found → soft-skip (exit 0, no failure ntfy)** — the next timer tick or promote retries. (A "CI not done yet" result is never treated as a deploy failure.)
...
- **Git auth** reuses the user's user-level git credential helper (the runner and timer both run as the user, who already pushes from this Mac). **`gh` auth for the CI gate differs by trigger**: the runner path uses the workflow `GITHUB_TOKEN` (`actions: read`), the timer path uses the user's `gh` keyring auth.
