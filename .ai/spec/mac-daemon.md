...
- **Peer filtering:** the daemon filters each chat.db row's effective peer — the **sender** for inbound rows, the **recipient** (resolved outbound peer) for outbound rows — against the Pi's canonicalized phone/email identifier set **before** forwarding any message. Only rows whose peer resolves to a known `contact_method` are emitted as `raw_message.*` events. Peers that are not in the CRM — spam, businesses, shortcodes, one-time confirmations, unsaved numbers, threads with strangers — never leave the Mac. This subsumes the prior plan's `messages_rematch` worker (no staged-but-unmatched rows exist) and eliminates the need for separate content-pattern or org-name heuristics.
...
- **Cold-start race recovery (30-day backwards scan):** when a `known-identifiers` refresh yields a newly-added identifier (computed by set-diff against the daemon's previous cache), the daemon performs a one-time backwards scan of chat.db over the last **30 days** for that sender's `handle.id` and forwards any matching rows. The 30-day window covers the "friend-of-a-friend met in a group chat, contact information saved after the fact" case: their prior messages get backdated into the CRM as soon as they appear in `contact_method`. Idempotency comes from the existing event-log `(source, source_id)` dedup — overlap with messages already forwarded via the live cursor is absorbed safely. The scan also reaches **outbound** rows the user sent in that handle's conversations: it resolves the handle's `chat_handle_join` memberships, then matches `is_from_me=1` rows by `chat_message_join` membership in those chats (outbound rows have a NULL `handle_id`, so they can only be tied to the handle via the chats it belongs to).
- **Outbound emission (`raw_message.sent`):** outbound messages (`message.is_from_me=1`) are emitted symmetrically with inbound, so texting a contact bumps `last_outreach_at` and the shared aggregator can form `mutual` sessions. Outbound rows usually carry a NULL `handle_id`, so their peer is resolved as: the joined `handle.id` when present (some macOS/SMS variants populate it on outbound rows), else the **first `chat_handle_join` entry by join ROWID** for the row's chat — which covers both 1:1 (the single peer) and group chats (the first member; a v1 simplification, so every outbound group message attributes to that one member). There is no self-handle exclusion in the lookup; it relies on macOS not inserting the account owner's own handle into `chat_handle_join`. The recipient-side known-identifiers filter applies (only messages to known contacts are emitted). A row with no resolvable peer, or no `chat.guid` (the Pi requires a non-empty `chat_id`), is skipped while the cursor still advances past it.
- **System / group-action row guard:** chat.db `message.item_type != 0` marks non-message rows (group rename, member add/remove, "you left the chat", etc.). Such rows are skipped when they carry no content — the guard is conjunctive (`item_type != 0` AND no text AND no attachment), so any content-bearing row is always kept regardless of `item_type`, and attachment presence is the `message_attachment_join` existence (not the optional attachment-metadata columns, which can be NULL on a real attachment). This applies to both directions and prevents user-initiated system rows from emitting as false `raw_message.sent` outreach bumps. Tapbacks/reactions are NOT filtered here (they are `item_type=0`); reactions stay deferred to the v2 mutation-scan reader. The reader's column whitelist includes `message.item_type` (schema label `chat_db_v2`).
- **One-time outbound re-backfill:** installs whose cursors advanced while inbound-only emission was in effect never emitted outbound rows. The cursor carries an additive `outbound_backfill_done` flag; the first tick after outbound emission ships resets the backfill walk once to re-cover the span down to the 2026-01-01 floor (re-emitted inbound rows dedup at the Pi; outbound rows emit for the first time), then sets the flag so it happens exactly once. A fresh install just sets the flag — its normal backfill already covers both directions. During the re-walk, `backfill_complete=false` is visible in status and new live messages may be delayed (never lost) while the shared tick budget drains — the same tradeoff as a fresh install's initial backfill.
