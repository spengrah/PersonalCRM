...
- Auto-apply at the start of the nightly QA round, before the export step, so every observation the round produces is priced from the current file by construction. Fail-open: warn loudly, emit a round status field (the round's stdout/`$GITHUB_ENV` emit channel — deliberately NOT the run-dir `manifest.json`, whose price-field writer is exactly the provenance machinery this spec deletes), never block the round.
...
- **Fail-open in the round, loud everywhere**: auto-apply and the cost assertion warn and emit round status fields (via the round's emit channel; see Goals — not the run-dir manifest) but never block a round; stale prices make cost approximate, a skipped round makes QA absent. The manual targets fail with non-zero exit.
