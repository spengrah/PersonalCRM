...
**Full stack validated end-to-end with the REAL artifacts** (the same files now in `infra/images/`, `infra/quadlet/`, `infra/caddy/`):

- **Both images build + run:** backend `distroless/static-debian12:nonroot` (136 MB, the two static binaries + `/migrations`); frontend `node:22-bookworm-slim` (312 MB, key-less standalone). Built in-VM with rootless `podman build`.
- **3-service stack via the real Quadlet units:** `daemon-reload` generated `personalcrm-{database,backend,frontend}.service` + `personalcrm-db-volume`/`crm-network`. Backend **migrated on boot**, reached `crm-postgres` over the shared `crm` network (aardvark DNS), and `/health` reported `database: healthy`. Frontend served. All `active`.
- **Resource limits apply** (cgroup-v2 delegation works for the system-account slice): `MemoryMax=536870912` (512M), `CPUQuotaPerSecUSec=1.5s` (150%). *Item resolved.*
- **Caddy `X-API-Key` edge-injection (revised Decision 3) works:** real Caddyfile validates; direct-to-backend no-key→401/with-key→200; **via Caddy with the client sending NO key → 200** (Caddy injected `{env.CRM_API_KEY}`), returning valid JSON; **control with no edge key → 401** (proves the 200 is from injection). The no-secret public image authenticates the whole stack.
- **Rollback drill:** `systemctl --user stop` the stack → 0 containers, the `personalcrm-db` volume persists; restart → backend `active`, **38 tables intact** (schema + data survive). Mirrors the cutover's "health-gate fails → stop containers → recover" path with zero data loss.

**Log-routing finding (firm):** `LogDriver=journald` is **incompatible with the podman-static bundle** — its `conmon` is built without journald support (`[conmon:e]: Include journald in compilation path…`), so a journald-driver container fails to start (exit 126). So container logs use the default **`k8s-file`** driver, accessed via **`podman logs crm-backend`** — NOT `journalctl -u …`. This changes the operator's log-access ergonomic (update the memory note). *Optional restore path:* install the distro `conmon` (apt, has journald) and point podman's `conmon_path` at it to re-enable `LogDriver=journald`; deferred as a minor follow-up. systemd still journals service-level start/stop/health events.

**Cutover env change surfaced:** the backend's `.env` `DATABASE_URL` host must become **`crm-postgres`** (the container name on the `crm` network), not `localhost`, after cutover.
