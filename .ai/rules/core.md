...
| App logic that changes behavior in a domain with a `spec/<domain>.yaml` file | Update the affected behaviors in `spec/<domain>.yaml` in the SAME PR — extend-in-place vs retire-and-mint per `spec/README.md` — and run `make spec-lint` + `make spec-coverage`. Every domain lists `ui` in `settled`, so a new/changed `surface: ui` then-item must land with its citing E2E test or an explicit waiver in the same PR (orphans block), and editing a cited behavior's then list must update citing indexes (out-of-range citations block) |
