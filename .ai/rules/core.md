...
| Waiting on a network event after re-visiting a view the tour/spec already loaded | A per-query `staleTime` OVERRIDES `defaultOptions.queries`, so the `isPlaywright ? 0 : 5min` escape hatch in `frontend/src/lib/query-client.ts` does NOT reach any hook that pins its own — `useContacts` (2m), `usePrefetchContact` (2m), `useOverdueContacts` (5m), `useContactIDs` (5m), all in `use-contacts.ts`. Those four are the ones this has BITTEN, NOT the full set — a dozen-plus more pins live across the calendar, tasks, imports, sync, merge, suggestions and interaction-queue hooks, so never rule this out just because your surface isn't contacts. A round trip that returns to the SAME query key (same sort/order/search/filter) within that window is served from cache and issues NO REQUEST AT ALL, so `waitForApi`/`waitForResponse` burns its full timeout. The symptom is indistinguishable from a listener-arming race, so `expectApi` "fixes" nothing — there is no response to arm for. Gate on the RENDERED consequence instead (the restored row, the pager resolving to "N of M"), which is correct whether the view came from cache or a cold fetch. Beware the inverse too: `waitForApi` PEEKS the shared buffer with `.find()` and does not splice (`capture.ts`), so a same-path wait with no intervening `capture()` (the only drain) resolves instantly on the PREVIOUS leg's response and gates on nothing |
