...
| `embedding` | Disposable derived storage: a `vector(1536)` keyed by a polymorphic no-FK `(target_kind, target_id)` (the closed CHECK enum names the source table: node/assertion/interaction/meeting_note/comms_message/telegram_message/messages_message) + `model_version`; PK `(target_kind, target_id, model_version)`; no envelope, no `deleted_at`, no vector index in SP1 (HNSW/IVFFlat deferred to SP3); supersedes the dormant `note_embedding`. Storage-only in SP1 (no generators yet) | `target_id` polymorphic (no FK) |
...
| `note_embedding` | Dormant vector-embedding table (`embedding vector(1536)` → note); never populated. Superseded by the graph-foundation `embedding` table (polymorphic, model-versioned) and pending removal — the historical sibling `interaction_embedding` was already dropped in migration 030 | → note |
...
**SP1 graph assertion events.** `AssertService` emits five `assertion.*` kinds — `assertion.proposed`, `assertion.accepted`, `assertion.superseded`, `assertion.rejected`, `assertion.provenance_added` — onto the same bus via `PublishTx`, idempotency-keyed on `<assertion_id>:<transition>` (and `<assertion_id>:provenance:<locator_hash>` for the many-per-assertion provenance kind). `assertion.accepted` and `assertion.superseded` route to ONE consumer — the `KnowledgeCacheUpdater` projection worker, which refreshes the `contact.location`/`birthday`/`how_met` derived cache columns from the current-accepted `lives_in`/`birthday`/`how_met` assertion (the bus routes by KIND, so the worker no-ops on any other predicate). `assertion.proposed`/`rejected`/`provenance_added` have no consumer. The change-feed, embedding, `relationship_signal`-recompute, and review-surface consumers are still SP3/SP4. A `Retract` emits `assertion.superseded` (closure_reason distinguishes it; there is no separate `assertion.retracted` kind), so the cache worker also fires on a retract of one of the three cutover predicates. The `KnowledgeCacheUpdater` recomputes from scratch (`GetCurrentAccepted` → column, or NULL when none), which is correct for supersession, closure, and retraction alike; `ContactService` additionally calls `KnowledgeCacheUpdater.RefreshTx` INLINE in its create/update/merge tx so a direct user edit's cache column is current on commit (no read-path gap waiting on the async worker).
...
| `contact.location` / `contact.birthday` / `contact.how_met` | `KnowledgeCacheUpdater` | Derived cache columns recomputed from the current-accepted `lives_in`/`birthday`/`how_met` assertion. `ContactService` create/update/merge + `EnrichmentService` emit assertions via `AssertService` and call `KnowledgeCacheUpdater.RefreshTx` inline; the async worker (on `assertion.accepted`/`superseded`) covers other producers. The contact SQL no longer writes these columns. |
