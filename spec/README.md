...
**Marker.** A citation is a line comment of the exact form `// spec: <ref>[, <ref> ...]` — literally `//`, one space, `spec:`, then one or more references separated by commas, where each reference is a behavior ID (`IMP-021`) or a then-item reference (`IMP-021[2]`, 0-based index into the behavior's `then` list). The marker must be the only content on its line — the scanner deliberately ignores prose that merely mentions a marker. To cite many references, either comma-separate them on one line or stack multiple `// spec:` lines. The same marker string is used on every test surface (Go `testing`, Playwright, Vitest, and any future MCP tests): a line comment is byte-identical across surfaces, inert (it cannot break compilation or a test run), and couples test code to no framework annotation API.
